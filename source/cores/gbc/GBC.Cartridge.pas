...
class function TGBCCartridge.DecodeCartridgeType(Code: Byte): TCartridgeType;
begin
...
constructor TGBCCartridge.Create(const Data: TBytes);
begin
  inherited Create;
  if Length(Data) < CartridgeHeaderSize then
    raise EGBCInvalidROM.CreateFmt('ROM is too short: %d bytes; header requires %d.', [Length(Data), CartridgeHeaderSize]);
...
function TGBCCartridge.GetHeaderChecksumValid: Boolean;
begin
  Result := FHeaderChecksum = FCalculatedHeaderChecksum;
end;

function TGBCCartridge.GetGlobalChecksumValid: Boolean;
begin
